...
if cfg.key not in info["devices"]:
    print(f"exiting. Nothing to do.  key {cfg.key} not found in yaml {cfg.yaml}")
    sys.exit()
print("---")
d = Device(netbox_obj, info["devices"][cfg.key])
d.create_or_update()
