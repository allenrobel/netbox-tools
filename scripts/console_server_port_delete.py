...
def get_parser():
    ...
    ex_port = f"{ex_prefix} --port myconsole_server_port"

    parser = argparse.ArgumentParser(
        description="DESCRIPTION: Netbox: Delete a console_server_port"
    )
...
def get_info():
    """
    return dictionary expected by ConsoleServerPort()
    """
    info = {}
    info["port"] = cfg.port
...
cfg = get_parser()
netbox_obj = netbox()
c = ConsoleServerPort(netbox_obj, get_info())
c.delete()
